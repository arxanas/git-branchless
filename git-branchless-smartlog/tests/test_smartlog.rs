...
fn test_show_signature_argument_parsing() -> eyre::Result<()> {
    ...
    assert!(help_stdout.contains("--show-signature"), 
    ...
    assert!(result.is_ok(), "The --show-signature flag should be recognized");
    
    // Test that the flag works when combined with other flags
    let result = git.branchless("smartlog", &["--show-signature", "--exact"]);
    assert!(result.is_ok(), "The --show-signature flag should work with --exact");
    
    let result = git.branchless("smartlog", &["--show-signature", "--reverse"]);
    assert!(result.is_ok(), "The --show-signature flag should work with --reverse");
