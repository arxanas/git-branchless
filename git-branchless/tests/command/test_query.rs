...
");
...

    {
        ...
        insta::assert_snapshot!(stderr, @r###"
        Evaluation error for expression 'foo()': no function with the name 'foo' could be found; these functions are available: all, ancestors, author.date, author.email, author.name, branches, children, committer.date, committer.email, committer.name, descendants, difference, draft, heads, intersection, message, none, not, nthancestor, nthparent, only, parents, paths.changed, range, roots, stack, union
        "###);
        insta::assert_snapshot!(stdout, @"");
    }
