...
fn get_child_output(
    ...
    let mut children: Vec<_> = current_node.children.iter().cloned().collect();
...
pub fn smartlog<Out: Write>(out: &mut Out) -> anyhow::Result<()> {
    ...
    let commit_metadata_providers: Vec<&dyn CommitMetadataProvider> = vec![
        &commit_oid_provider,
        &relative_time_provider,
        &branches_provider,
        &differential_revision_provider,
    ...
    render_graph(
        ...
        &graph,
        &HeadOid(head_oid),
        commit_metadata_providers.as_slice(),
    )?;
