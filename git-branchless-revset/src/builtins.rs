...
fn fn_branches(
    ...
    let branch_commits = make_pattern_matcher(
        ...
        commitset_hint.map_or_else(
            || Some(ctx.dag.branch_commits.clone()),
            |hint| Some(hint.intersection(&ctx.dag.branch_commits)),
        ),
    )?;
...
/// Make a pattern matcher that operates on all visible commits if a specific
/// set of commits is not provided.
